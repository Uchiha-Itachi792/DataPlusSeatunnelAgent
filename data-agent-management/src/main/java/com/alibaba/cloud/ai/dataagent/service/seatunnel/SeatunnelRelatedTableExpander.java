...
/**
 * SeaTunnel 同步：根据外键、逻辑外键与用户过滤描述，自动扩展所需的关联表（与 SQL 同步轨 {@code SyncRelatedTableExpander} 隔离）。
 */
@Slf4j
@Component
@AllArgsConstructor
public class SeatunnelRelatedTableExpander {
	...
	public List<String> expand(Integer datasourceId, String sourceTable, String userInput,
		...
		if (!seatunnelProperties.getRelatedTable().isFkAutoExpandEnabled()) {
		...
		for (ForeignKeyInfoBO fk : loadForeignKeys(datasourceId, sourceTable)) {
			if (sourceLower.equalsIgnoreCase(fk.getTable()) && shouldIncludeReferencedTable(userInput, fk)) {
				...
				log.info("SeaTunnel auto-included related table {} via FK {}.{} -> {}.{}", fk.getReferencedTable(),
		...
		for (LogicalRelation relation : datasourceService.getLogicalRelations(datasourceId)) {
			...
			if (sourceLower.equalsIgnoreCase(relation.getSourceTableName())
				...
				log.info("SeaTunnel auto-included related table {} via logical relation from {}",
						relation.getTargetTableName(), relation.getSourceTableName());
			}
		}
	...
	private List<ForeignKeyInfoBO> loadForeignKeys(Integer datasourceId, String sourceTable) {
		try {
			...
			log.warn("Failed to load foreign keys for SeaTunnel table {} in datasource {}: {}", sourceTable,
