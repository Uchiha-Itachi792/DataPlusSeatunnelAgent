...
/**
 * 基于召回 Schema 与用户输入，由 LLM 决策 SeaTunnel 同步任务的物理表名（与 SQL 同步轨隔离）。
 */
@Slf4j
@Service
@AllArgsConstructor
public class SeatunnelTableResolveService {
	 ...
	 */
	public SeatunnelTableResolveDTO resolve(String recallQuery, String multiTurn, SchemaDTO schemaDTO, String evidence) {
		if (schemaDTO == null || schemaDTO.getTable() == null || schemaDTO.getTable().isEmpty()) {
			return null;
		}
		String prompt = PromptHelper.buildSeatunnelTableResolvePrompt(multiTurn, recallQuery, schemaDTO, evidence);
		String llmOutput = llmService.blockToString(llmService.callUser(prompt));
		if (!StringUtils.hasText(llmOutput)) {
			return null;
		}
		SeatunnelTableResolveDTO resolved = jsonParseUtil.tryConvertToObject(llmOutput, SeatunnelTableResolveDTO.class);
		if (resolved == null || !StringUtils.hasText(resolved.getSourceTable())
				|| !StringUtils.hasText(resolved.getTargetTable())) {
			log.warn("SeaTunnel table resolve incomplete for input: {}", recallQuery);
			return null;
		}
		log.info("Resolved SeaTunnel sync tables: source={}, target={}, related={}", resolved.getSourceTable(),
